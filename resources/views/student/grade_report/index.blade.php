...
@php
    ...
    $s1ExamScores = $semester1Scores ?? collect();
    $s2ExamScores = $semester2Scores ?? collect();

    $s1ExamAvg = collect($s1ExamScores)->count() ? round(collect($s1ExamScores)->avg('score'), 2) : null;
    $s2ExamAvg = collect($s2ExamScores)->count() ? round(collect($s2ExamScores)->avg('score'), 2) : null;
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-2xl shadow-sm border p-6 mb-8">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                <div>
                    <p class="text-sm font-semibold text-blue-600">Student Academic Portal</p>
                    <h1 class="text-3xl font-bold text-gray-900 mt-1">Grade Report</h1>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <select id="gradeSelect"
                            class="px-5 py-2.5 min-w-[220px] border rounded-xl bg-white font-semibold">
            ...
            @foreach([
                ...
                <div class="bg-white rounded-2xl shadow-sm border overflow-hidden">
                    ...
                    <div class="p-8 text-center">
                        ...
                        @else
                            <p class="text-gray-500 font-semibold">Waiting for complete scores</p>
                        @endif
                    </div>
                </div>
            @endforeach
        ...
        {{-- Semester Exam Tables --}}
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-20">
            @foreach([
                ['title' => '1st Semester Exam Scores', 'scores' => $s1ExamScores, 'avg' => $s1ExamAvg],
                ['title' => '2nd Semester Exam Scores', 'scores' => $s2ExamScores, 'avg' => $s2ExamAvg],
            ] as $exam)
                <div class="bg-white rounded-2xl shadow-sm border overflow-hidden">
...
<script>
    document.getElementById('gradeSelect').addEventListener('change', function () {
        window.location.href = `{{ route('student.scores') }}?grade_level=${this.value}`;
    });
</script>
@endsection
