...
<div class="py-8 bg-gray-50 min-h-screen">
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        ...
        <form action="{{ route('student.setting.update') }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email', auth()->user()->email) }}"
                               class="border border-gray-300 rounded px-3 py-2 w-full focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-gray-100 cursor-not-allowed" required readonly>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="tel" name="phone_number" value="{{ old('phone_number', $student->phone_number ?? '') }}"
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="bg-gray-50 p-4 rounded border border-gray-200">
                        ...
                        <p class="font-semibold text-gray-900">{{ $student->class->name ?? 'Not Assigned' }}</p>
                    </div>
                    <div class="bg-gray-50 p-4 rounded border border-gray-200">
                        <p class="text-sm text-gray-600 mb-1">Student ID</p>
                        <p class="font-semibold text-gray-900">{{ $student->student_id ?? 'N/A' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-blue-900">
                    <div>
                        <span class="text-blue-600">Student ID:</span> {{ $student->student_id ?? '#' . $student->id }}
                    </div>
                    <div>
                        <span class="text-blue-600">Member Since:</span> {{ auth()->user()->created_at->format('M d, Y') }}
                    </div>
                </div>
            </div>
            ...
            <div class="flex gap-3 pt-4">
                ...
                <a href="{{ route('dashboard.student') }}" class="bg-gray-400 hover:bg-gray-500 text-white px-8 py-2 rounded-lg font-semibold transition flex items-center gap-2">
