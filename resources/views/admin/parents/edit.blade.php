...
const selectedBox = document.getElementById('selected_students_box');
const selectedInputs = document.getElementById('selected_students_inputs');
...
function addStudent(id,text){

    if(selectedStudents.includes(String(id))){
        return;
    }

    selectedStudents.push(String(id));

    const badge=document.createElement('div');

    badge.className=
    'flex items-center gap-2 bg-blue-100 text-blue-700 px-3 py-2 rounded-full';

    badge.dataset.studentId=id;
...
addBtn.addEventListener('click',function(){
    ...
    const text=studentSearch.value.trim();
...
selectedBox.addEventListener('click',function(e){
    ...
    if(e.target.classList.contains('remove-btn')){
        ...
        const badge=e.target.closest('[data-student-id]');

        const id=badge.dataset.studentId;
