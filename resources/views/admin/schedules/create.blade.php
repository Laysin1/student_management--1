...
<div class="container mx-auto px-6 py-8 max-w-3xl">
  <h1 class="text-2xl font-bold mb-2">Add Schedule</h1>
  <p class="text-gray-600 mb-6">Upload a schedule image for a class or teacher.</p>
  ...
  <form action="{{ route('schedules.store') }}" method="POST" enctype="multipart/form-data" class="bg-white shadow rounded-lg p-6 space-y-5">
    @csrf

    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        <label class="font-semibold text-gray-800 mb-2 block">Title</label>
        <input type="text" name="title" value="{{ old('title') }}" class="border border-gray-300 rounded px-3 py-2 w-full">
      </div>
      <div>
    ...
    <div id="classSelect" class="{{ old('type') === 'teacher' ? 'hidden' : '' }}">
    ...
    <div id="teacherSelect" class="{{ old('type') === 'class' ? 'hidden' : '' }}">
    ...
    <div>
      <label class="font-semibold text-gray-800 mb-2 block">Schedule Image <span class="text-red-500">*</span></label>
      <input type="file" name="photo" accept="image/*" class="border border-gray-300 rounded px-3 py-2 w-full" required>
      <small class="text-gray-500">PNG/JPG up to 4 MB</small>
    </div>
